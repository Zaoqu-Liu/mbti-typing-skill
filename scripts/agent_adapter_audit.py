...
def check_docs_and_visual(root: Path) -> list[Check]:
    ...
    return [
        ...
        Check("exporter:contract", contains_all(exporter, ("PACK_SCHEMA", "BASELINE_PATHS", "AGENT_PACK_MANIFEST.json", "destination is not empty", "core_targets")), "pack exporter has schema, baseline, core alias, manifest, and write guard"),
        Check("pack_audit:contract", contains_all(pack_audit, ("Agent Pack Export Audit", "core_run", "dry_run", "all_export", "selective_export", "unknown_target")), "pack export audit covers core, dry-run, all, selective, and unknown-target flows"),
        ...
        Check("compat_svg:labels", contains_all(compatibility_svg, required_tool_terms + ("4 core adapters", "optional recipes", "one protocol", "AGENTS.md", "agent_adapter_audit.py")), "compatibility grid contains expected labels"),
        ...
        Check("pack_svg:labels", contains_all(pack_svg, ("Agent Pack Export Flow", "scripts/export_agent_pack.py", "agent-adapters/manifest.json", "AGENT_PACK_MANIFEST.json", "scripts/agent_pack_export_audit.py", "core_targets", "make test", "Target Repo")), "pack export flow contains expected labels"),
        ...
        Check("lab_svg:labels", contains_all(lab_svg, ("Agent Adapter Lab Flow", "agent-adapters/manifest.json", "Target selector", "Pack command", "AGENT_PACK_MANIFEST.json", "scripts/export_agent_pack.py", "scripts/agent_adapter_lab_audit.py", "agent_adapter_improvement.yml", "4 core", "optional recipes")), "Agent Adapter Lab flow contains expected labels"),
